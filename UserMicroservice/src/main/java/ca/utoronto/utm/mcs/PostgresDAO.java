...
public class PostgresDAO {
    ...
    // implement the register database operation here
    public void postUserRegister(String email, String password, String prefer_name) throws SQLException {
        String query = "INSERT INTO users(email,password,prefer_name,rides,availablecoupons,redeemedcoupons) Values('%s','%s','%s', 0,ARRAY[]::integer[],ARRAY[]::integer[])";
        query = String.format(query, email, password, prefer_name);
        this.st.execute(query);
    }

    // implement the login database operation here
